...
(function() {
    ...
    BX.Sale.OrderAjaxComponent = {
        ...
        init: function(parameters) {
            ...
            if (!this.result.IS_AUTHORIZED || this.result.LAST_ORDER_DATA.FAIL) this.initFirstSection();
            this.initOptions();
            this.editOrder();
            this.bindEvents();
        ...
        getAllFormData: function() {
            ...
            for (i in prepared.data) if (prepared.data.hasOwnProperty(i) && i == "") delete prepared.data[i];
        ...
        endLoader: function(loaderTimer) {
            ...
            if (this.loadingScreen && this.loadingScreen.isShown()) this.loadingScreen.close();
        ...
        animateScrollTo: function(node, duration, shiftToTop) {
            if (!node) return;
            var scrollTop = BX.GetWindowScrollPos().scrollTop, orderBlockPos = BX.pos(this.orderBlockNode), ghostTop = BX.pos(node).top - (this.isMobile ? 50 : 0);
            if (shiftToTop) ghostTop -= parseInt(shiftToTop);
            if (ghostTop + window.innerHeight > orderBlockPos.bottom) ghostTop = orderBlockPos.bottom - window.innerHeight + 17;
            new BX.easing({
        ...
        checkKeyPress: function(event) {
            if (event.keyCode == 13) {
                var target = event.target || event.srcElement, send = target.getAttribute("data-send"), nextAttr, next;
                if (!send) {
        ...
        getSizeString: function(maxSize, len) {
            var gbDivider = 1024 * 1024 * 1024, mbDivider = 1024 * 1024, kbDivider = 1024, str;
            maxSize = parseInt(maxSize);
            len = parseInt(len);
            if (maxSize > gbDivider) str = parseFloat(maxSize / gbDivider).toFixed(len) + " Gb"; else if (maxSize > mbDivider) str = parseFloat(maxSize / mbDivider).toFixed(len) + " Mb"; else if (maxSize > kbDivider) str = parseFloat(maxSize / kbDivider).toFixed(len) + " Kb"; else str = maxSize + " B";
            return str;
        },
        getFileAccepts: function(accepts) {
            var arr = [], arAccepts = accepts.split(","), i, currentAccept;
            var mimeTypesMap = {
        ...
        checkNotifications: function() {
            var informer = this.mainErrorsNode.querySelector('[data-type="informer"]'), success, sections, className, text, scrollTop, informerPos;
            if (informer) {
                if (this.firstLoad && this.result.IS_AUTHORIZED && !this.result.LAST_ORDER_DATA.FAIL) {
                    ...
                    informer.appendChild(BX.create("DIV", {
                        props: {
                            className: "row"
                        },
        ...
        checkBlockErrors: function(node) {
            ...
            if (showError) BX.addClass(node, "bx-step-error"); else if (showWarning) BX.addClass(node, "bx-step-warning"); else BX.removeClass(node, "bx-step-error bx-step-warning");
            return !showError;
        },
        ...
        showWarnings: function() {
            var sections = this.orderBlockNode.querySelectorAll("div.bx-soa-section.bx-active"), currentDelivery = this.getSelectedDelivery(), k, warningString;
            ...
            if (currentDelivery && currentDelivery.CALCULATE_ERRORS) {
                ...
                if (this.params.MESS_DELIVERY_CALC_ERROR_TEXT.length) warningString += "<br /><small>" + BX.util.htmlspecialchars(this.params.MESS_DELIVERY_CALC_ERROR_TEXT) + "</small>";
        ...
        showBlockWarning: function(node, warnings, hide) {
            ...
            if (errorNode) {
                if (BX.type.isString(warnings)) warnStr = warnings; else {
                    for (i in warnings) if (warnings.hasOwnProperty(i)) warnStr += warnings[i] + "<br />";
                }
        ...
        showPagination: function(entity, node) {
            ...
            if (pagination.pages.length > 1) {
                navigation.push(BX.create("LI", {
                    ...
                    html: pagination.pageNumber == 1 ? "<span>" + BX.util.htmlspecialchars(this.params.MESS_NAV_BACK) + "</span>" : '<a href=""><span>' + BX.util.htmlspecialchars(this.params.MESS_NAV_BACK) + "</span></a>",
                    events: {
                ...
                navigation.push(BX.create("LI", {
                    ...
                    html: pagination.pageNumber == pagination.pages.length ? "<span>" + BX.util.htmlspecialchars(this.params.MESS_NAV_FORWARD) + "</span>" : '<a href=""><span>' + BX.util.htmlspecialchars(this.params.MESS_NAV_FORWARD) + "</span></a>",
                    events: {
                        click: BX.proxy(this.doPagination, this)
                    }
                }));
        ...
        doPagination: function(e) {
            var target = e.target || e.srcElement, node = target.tagName == "LI" ? target : BX.findParent(target, {
                tagName: "LI"
            ...
            if (entity == "delivery") this.showDeliveryItemsPage(page); else if (entity == "paySystem") this.showPaySystemItemsPage(page); else if (entity == "pickUp") this.showPickUpItemsPage(page);
            ...
            if (BX.saleOrderAjax && this.result.ORDER_PROP && this.result.ORDER_PROP.properties) {
                ...
                for (i = 0; i < this.result.ORDER_PROP.properties.length; i++) {
                    ...
                    if (curProp.TYPE == "LOCATION" && curProp.MULTIPLE == "Y" && curProp.IS_LOCATION != "Y") {
                        ...
                        if (curProp.IS_LOCATION == "Y") this.deliveryLocationInfo.loc = curProp.ID;
                        if (curProp.IS_ZIP == "Y") {
                            attrObj.isZip = true;
        ...
        reachGoal: function(goal, section) {
            var counter = this.params.YM_GOALS_COUNTER || "", useGoals = this.params.USE_YM_GOALS == "Y" && typeof window["yaCounter" + counter] !== "undefined", goalId;
            if (useGoals) {
        ...
        isPriceChanged: function(result) {
            var priceBefore = this.result.TOTAL.ORDER_TOTAL_LEFT_TO_PAY === null || this.result.TOTAL.ORDER_TOTAL_LEFT_TO_PAY === "" ? this.result.TOTAL.ORDER_TOTAL_PRICE : this.result.TOTAL.ORDER_TOTAL_LEFT_TO_PAY, priceAfter = result.order.TOTAL.ORDER_TOTAL_LEFT_TO_PAY === null ? result.order.TOTAL.ORDER_TOTAL_PRICE : result.order.TOTAL.ORDER_TOTAL_LEFT_TO_PAY;
        ...
        initPickUpPagination: function() {
            var usePickUpPagination = false, usePickUp = false, stores, i = 0, arReserve, pages, arPages;
        ...
        locationsCompletion: function() {
            ...
            for (i in this.locations) {
                ...
                if (clearButton) {
                    BX.bind(clearButton, "click", function(e) {
                        var target = e.target || e.srcElement, parent = BX.findParent(target, {
                            tagName: "DIV",
            ...
            if (this.firstLoad && this.result.IS_AUTHORIZED && !this.result.LAST_ORDER_DATA.FAIL) this.showActualBlock();
            this.checkNotifications();
            if (this.activeSectionId !== this.regionBlockNode.id) this.editFadeRegionContent(this.regionBlockNode.querySelector(".bx-soa-section-content"));
            if (this.activeSectionId != this.propsBlockNode.id) this.editFadePropsContent(this.propsBlockNode.querySelector(".bx-soa-section-content"));
        },
        fixLocationsStyle: function(section, hiddenSection) {
            if (!section || !hiddenSection) return;
            var regionActive = this.activeSectionId == section.id ? section : hiddenSection, locationSearchInputs, locationStepInputs, i;
            ...
            if (locationSearchInputs.length) for (i = 0; i < locationSearchInputs.length; i++) BX.addClass(locationSearchInputs[i], "form-control");
        ...
        clickNextAction: function(event) {
            var target = event.target || event.srcElement, actionSection = BX.findParent(target, {
                className: "bx-active"
            }), section = this.getNextSection(actionSection), allSections, titleNode, editStep;
        ...
        getNextSection: function(actionSection, skippedSection) {
            ...
            var allSections = this.orderBlockNode.querySelectorAll(".bx-soa-section.bx-active"), nextSection, i, onePickUp, oneDelivery, onePaySystem, delivery, titleNode;
                ...
                if (allSections[i].id == actionSection.id && allSections[i + 1]) {
                    ...
                    if (this.params.SKIP_USELESS_BLOCK == "Y") {
                        if (nextSection.id == this.pickUpBlockNode.id) {
                            if (delivery = this.getSelectedDelivery()) onePickUp = this.getPickUpInfoArray(delivery.STORE).length == 1;
                        }
        ...
        getPrevSection: function(actionSection) {
            if (!this.orderBlockNode || !actionSection) return;
            var allSections = this.orderBlockNode.querySelectorAll(".bx-soa-section.bx-active"), prevSection, i, onePickUp, delivery;
            for (i = 0; i < allSections.length; i++) if (allSections[i].id == actionSection.id && allSections[i - 1]) {
                onePickUp = false;
        ...
        addAnimationEffect: function(node, className, timeout) {
            ...
            this.timeOut[node.id] = {
                className: className,
                timer: setTimeout(BX.proxy(function() {
        ...
        fade: function(node, nextSection) {
            ...
            if (nextSection) {
                var windowScrollTop = BX.GetWindowScrollPos().scrollTop, orderPos = BX.pos(this.orderBlockNode), nodePos = BX.pos(node), diff, scrollTo, nextSectionHeightBefore, nextSectionHeightAfter, nextSectionHidden, offset;
                ...
                if (offset > 0) scrollTo = orderPos.top - offset / 2; else {
                    if (nodePos.top > windowScrollTop) scrollTo = nodePos.top; else scrollTo = nodePos.bottom + 6 - objHeightOrig + objHeight;
                    if (scrollTo + window.innerHeight > orderPos.bottom + 25 + diff) scrollTo = orderPos.bottom + 25 + diff - window.innerHeight;
                }
                scrollTo -= this.isMobile ? 50 : 0;
            }
            new BX.easing({
                duration: nextSection ? 1e3 : 600,
                start: {
        ...
        showByClick: function(event) {
            var target = event.target || event.srcElement, showNode = BX.findParent(target, {
                className: "bx-active"
            }), fadeNode = BX(this.activeSectionId), scrollTop = BX.GetWindowScrollPos().scrollTop;
        ...
        getBlockFooter: function(node) {
            var sections = this.orderBlockNode.querySelectorAll(".bx-soa-section.bx-active"), firstSection = sections[0], lastSection = sections[sections.length - 1], currentSection = BX.findParent(node, {
                className: "bx-soa-section"
            }), isLastNode = false, buttons = [];
        ...
        switchOrderSaveButtons: function(state) {
            var orderSaveNode = this.orderSaveBlockNode, totalButton = this.totalBlockNode.querySelector(".bx-soa-cart-total-button-container"), mobileButton = this.mobileTotalBlockNode.querySelector(".bx-soa-cart-total-button-container"), lastState = this.orderSaveBlockNode.style.display == "";
            if (lastState != state) {
                if (state) {
        ...
        changeVisibleContent: function() {
            ...
            if (!this.result.IS_AUTHORIZED && this.params.SHOW_ORDER_BUTTON == "final_step") this.switchOrderSaveButtons(sections[sections.length - 1].getAttribute("data-visited") == "true");
        },
        ...
        editOrder: function() {
            ...
            for (i in sections) if (sections.hasOwnProperty(i)) this.editSection(sections[i]);
            this.editTotalBlock();
        ...
        editSection: function(section) {
            if (!section || !section.id) return;
            if (this.result.SHOW_AUTH && section.id != this.authBlockNode.id && section.id != this.basketBlockNode.id) section.style.display = "none"; else if (section.id != this.pickUpBlockNode.id) section.style.display = "";
            var active = section.id == this.activeSectionId, titleNode = section.querySelector(".bx-soa-section-title-container"), editButton, errorContainer;
        ...
        editAuthBlock: function() {
            ...
            var authContent = this.authBlockNode.querySelector(".bx-soa-section-content"), regContent, okMessageNode;
        ...
        editRegistrationForm: function(authContent) {
            ...
            authFormNodes.push(this.createAuthFormInputContainer(BX.message("STOF_LASTNAME"), BX.create("INPUT", {
                attrs: {
                    "data-next": "NEW_EMAIL"
                },
                props: {
            ...
            authFormNodes.push(this.createAuthFormInputContainer(BX.message("STOF_EMAIL"), BX.create("INPUT", {
                attrs: {
            ...
            if (this.result.AUTH.new_user_registration_email_confirmation != "Y") {
                authFormNodes.push(BX.create("LABEL", {
                    props: {
                        for: "NEW_GENERATE_N"
                    },
                    children: [ BX.create("INPUT", {
            ...
            authFormNodes.push(BX.create("DIV", {
                ...
                }), true), this.createAuthFormInputContainer(BX.message("STOF_PASSWORD"), BX.create("INPUT", {
                    props: {
                        name: "NEW_PASSWORD",
                        type: "password",
                        size: 30
                ...
                }), true), this.createAuthFormInputContainer(BX.message("STOF_RE_PASSWORD"), BX.create("INPUT", {
                    props: {
        ...
        editSocialContent: function(authContent) {
            ...
            var nodes = [], socServiceHiddenNode = BX("bx-soa-soc-auth-services").querySelector(".bx-authform-social");
            ...
            nodes.push(BX.create("DIV", {
                ...
                children: [ BX.create("P", {
                    text: this.params.MESS_REGISTRATION_REFERENCE
                }), BX.create("A", {
                    props: {
        ...
        getAuthReference: function(authContent) {
            authContent.appendChild(BX.create("DIV", {
                ...
                children: [ BX.create("DIV", {
                    ...
                    children: [ BX.util.htmlspecialchars(this.params.MESS_AUTH_REFERENCE_1), BX.create("BR"), BX.util.htmlspecialchars(this.params.MESS_AUTH_REFERENCE_2), BX.create("BR"), BX.util.htmlspecialchars(this.params.MESS_AUTH_REFERENCE_3) ]
                }) ]
            }));
        },
        toggleAuthForm: function(event) {
            if (!event) return;
            var target = event.target || event.srcElement, section = BX.findParent(target, {
                className: "bx-soa-section"
            ...
            section.appendChild(BX.create("DIV", {
                props: {
        ...
        alignBasketColumns: function() {
            ...
            var i = 0, k, columns = 0, columnNodes, windowSize = BX.GetWindowInnerSize(), basketRows, percent;
        ...
        editActiveBasketBlock: function(activeNodeMode) {
            ...
            if (this.initialized.basket) {
                ...
                if (this.params.SHOW_COUPONS_BASKET == "Y") this.editCoupons(basketContent);
        ...
        editFadeBasketBlock: function(activeNodeMode) {
            ...
            if (this.initialized.basket) {
                ...
                if (this.params.SHOW_COUPONS_BASKET == "Y") this.editCouponsFade(newContent);
        ...
        editBasketItems: function(basketItemsNode, active) {
            ...
            var index = 0, i;
            if (this.params.SHOW_BASKET_HEADERS == "Y") this.editBasketItemsHeader(basketItemsNode);
            for (i in this.result.GRID.ROWS) if (this.result.GRID.ROWS.hasOwnProperty(i)) this.createBasketItem(basketItemsNode, this.result.GRID.ROWS[i], index++, !!active);
        },
        editBasketItemsHeader: function(basketItemsNode) {
            if (!basketItemsNode) return;
            var headers = [ BX.create("DIV", {
                props: {
            ...
            for (i = 0; i < this.result.GRID.HEADERS.length; i++) {
                ...
                if (column.id == "NAME" || column.id == "PREVIEW_PICTURE" || column.id == "PROPS" || column.id == "NOTES") continue;
        ...
        createBasketItem: function(basketItemsNode, item, index, active) {
            var mainColumns = [], otherColumns = [], hiddenColumns = [], currentColumn, basketColumnIndex = 0, i, tr, cols;
            if (this.options.showPreviewPicInBasket || this.options.showDetailPicInBasket) mainColumns.push(this.createBasketItemImg(item.data));
            mainColumns.push(this.createBasketItemContent(item.data));
            for (i = 0; i < this.result.GRID.HEADERS.length; i++) {
                currentColumn = this.result.GRID.HEADERS[i];
                if (currentColumn.id == "NAME" || currentColumn.id == "PREVIEW_PICTURE" || currentColumn.id == "PROPS" || currentColumn.id == "NOTES") continue;
            ...
            if (active) {
                for (i = 0; i < this.result.GRID.HEADERS_HIDDEN.length; i++) {
                    ...
                    if (BX.type.isArray(tr)) hiddenColumns = hiddenColumns.concat(tr); else if (tr) hiddenColumns.push(tr);
                }
            }
            cols = [ BX.create("DIV", {
                props: {
        ...
        showAdditionalProperties: function(event) {
            var target = event.target || event.srcElement, infoContainer = target.nextSibling, parentContainer = BX.findParent(target, {
                className: "bx-soa-item-tr bx-soa-item-info-container"
        ...
        createBasketItemImg: function(data) {
            ...
            if (data.PREVIEW_PICTURE_SRC && data.PREVIEW_PICTURE_SRC.length) logotype = this.getImageSources(data, "PREVIEW_PICTURE"); else if (data.DETAIL_PICTURE_SRC && data.DETAIL_PICTURE_SRC.length) logotype = this.getImageSources(data, "DETAIL_PICTURE");
            ...
            if (data.DETAIL_PAGE_URL && data.DETAIL_PAGE_URL.length) logoNode = BX.create("A", {
                props: {
        ...
        createBasketItemContent: function(data) {
            var itemName = data.NAME || "", titleHtml = BX.util.htmlspecialchars(itemName), props = data.PROPS || [], propsNodes = [];
            if (data.DETAIL_PAGE_URL && data.DETAIL_PAGE_URL.length) titleHtml = '<a href="' + data.DETAIL_PAGE_URL + '">' + titleHtml + "</a>";
        ...
        createBasketItemColumn: function(column, allData, active) {
            ...
            var data = allData.columns[column.id] ? allData.columns : allData.data, toRight = BX.util.in_array(column.id, [ "QUANTITY", "PRICE_FORMATED", "DISCOUNT_PRICE_PERCENT_FORMATED", "SUM" ]), textNode = BX.create("DIV", {
                props: {
                    className: "bx-soa-item-td-text"
                }
            }), logotype = this.getImageSources(allData.data, "DETAIL_PICTURE"), img;
            if (column.id == "PRICE_FORMATED") {
            ...
            } else if (column.id == "DISCOUNT") textNode.appendChild(BX.create("STRONG", {
                props: {
                    className: "bx-price"
                },
                ...
                if (logotype && logotype.src_1x && logotype.src_orig) BX.bind(img, "click", BX.delegate(function(e) {
                ...
                if (BX.type.isArray(columnData)) {
                    for (var i in columnData) {
                        if (columnData.hasOwnProperty(i)) {
                            if (columnData[i].type == "image") val.push(this.getImageContainer(columnData[i].value, columnData[i].source)); else if (columnData[i].value) {
                ...
                } else if (columnData) textNode.appendChild(BX.create("SPAN", {
        ...
        createBasketItemHiddenColumn: function(column, allData) {
            if (!column || !allData) return;
            var data = allData.columns[column.id] ? allData.columns : allData.data, textNode = BX.create("TD", {
                props: {
                    className: "bx-soa-info-text"
                }
            }), logotype = this.getImageSources(allData.data, "DETAIL_PICTURE"), img, i;
            if (column.id == "PROPS") {
            ...
            } else if (column.id == "SUM") textNode.appendChild(BX.create("STRONG", {
                props: {
                    className: "bx-price all"
                },
                text: data.SUM
                        ...
                        BX.create("IMG", {
                            ...
                            events: {
                                load: function() {
                                    ...
                                    if (content) {
                                        var windowSize = BX.GetWindowInnerSize(), ratio = this.isMobile ? .5 : .9, contentHeight, contentWidth;
        ...
        editCoupons: function(basketItemsNode) {
            var couponsList = this.getCouponsList(true), couponsLabel = this.getCouponsLabel(true), couponsBlock = BX.create("DIV", {
                props: {
        ...
        editCouponsFade: function(basketItemsNode) {
            ...
            var couponsList = this.getCouponsList(false), couponsLabel, couponsBlock;
        ...
        getCouponNode: function(coupon, active) {
            var couponName = BX.util.htmlspecialchars(coupon.text) || "", couponDesc = coupon.desc && coupon.desc.length ? coupon.desc.charAt(0).toUpperCase() + coupon.desc.slice(1) : BX.message("SOA_NOT_FOUND"), couponStatus = coupon.status || "BAD", couponItem, tooltip;
            ...
            return BX.create("STRONG", {
                ...
                children: active ? [ couponName || "", BX.create("SPAN", {
                    ...
                    events: {
                        click: BX.proxy(function(e) {
                            var target = e.target || e.srcElement, coupon = BX.findParent(target, {
                                tagName: "STRONG"
                            });
                            if (coupon && coupon.getAttribute("data-coupon")) this.sendRequest("removeCoupon", coupon.getAttribute("data-coupon"));
                        }, this)
                    }
                }), BX.create("SPAN", {
                    props: {
        ...
        addCoupon: function(coupon) {
            var couponListNodes = this.orderBlockNode.querySelectorAll(".bx-soa-coupon:not(.bx-soa-coupon-item-fixed) .bx-soa-coupon-item"), couponInput = BX("coupon"), i;
            for (i = 0; i < couponListNodes.length; i++) couponListNodes[i].appendChild(this.getCouponNode({
                text: coupon
        ...
        removeCoupon: function(coupon) {
            ...
            for (i in couponNodes) if (couponNodes.hasOwnProperty(i)) BX.remove(couponNodes[i]);
        },
        ...
        editActiveRegionBlock: function(activeNodeMode) {
            var node = activeNodeMode ? this.regionBlockNode : this.regionHiddenBlockNode, regionContent, regionNode, regionNodeCol;
        ...
        editFadeRegionContent: function(node) {
            ...
            var selectedPersonType = this.getSelectedPersonType(), errorNode = this.regionHiddenBlockNode.querySelector(".alert.alert-danger"), addedHtml = "", props = [], locationProperty, input, zipValue = "", zipProperty, fadeParamName, i, k, locationString, validRegionErrors;
            BX.cleanNode(node);
            if (errorNode) node.appendChild(errorNode.cloneNode(true));
            if (selectedPersonType && selectedPersonType.NAME && BX.util.object_keys(this.result.PERSON_TYPE).length > 1) addedHtml += "<span>" + BX.util.htmlspecialchars(this.params.MESS_PERSON_TYPE) + ":</span> " + selectedPersonType.NAME + "<br />";
            ...
            for (i in this.result.ORDER_PROP.properties) {
                if (this.result.ORDER_PROP.properties.hasOwnProperty(i)) {
                    if (this.result.ORDER_PROP.properties[i].IS_LOCATION == "Y") locationProperty = this.result.ORDER_PROP.properties[i]; else if (this.result.ORDER_PROP.properties[i].IS_ZIP == "Y") {
            ...
            if (locationProperty && locationString.length) addedHtml += "<span>" + BX.util.htmlspecialchars(locationProperty.NAME) + ":</span> " + locationString + "<br />";
            if (zipProperty && zipValue.length) addedHtml += "<span>" + BX.util.htmlspecialchars(zipProperty.NAME) + ":</span> " + zipValue;
        ...
        getSelectedPersonType: function() {
            var personTypeInput, currentPersonType, personTypeId, i, personTypeLength = BX.util.object_keys(this.result.PERSON_TYPE).length;
            if (personTypeLength == 1) {
                personTypeInput = this.regionBlockNode.querySelector("input[type=hidden][name=PERSON_TYPE]");
                if (!personTypeInput) personTypeInput = this.regionHiddenBlockNode.querySelector("input[type=hidden][name=PERSON_TYPE]");
            } else if (personTypeLength == 2) {
                personTypeInput = this.regionBlockNode.querySelector("input[type=radio][name=PERSON_TYPE]:checked");
                if (!personTypeInput) personTypeInput = this.regionHiddenBlockNode.querySelector("input[type=radio][name=PERSON_TYPE]:checked");
            } else {
                personTypeInput = this.regionBlockNode.querySelector("select[name=PERSON_TYPE] > option:checked");
                if (!personTypeInput) personTypeInput = this.regionHiddenBlockNode.querySelector("select[name=PERSON_TYPE] > option:checked");
            }
            if (personTypeInput) {
        ...
        getLocationString: function(node) {
            if (!node) return "";
            var locationInputNode = node.querySelector(".bx-ui-sls-route"), locationString = "", locationSteps, i, altLoc;
        ...
        getPersonTypeControl: function(node) {
            ...
            var personTypesCount = BX.util.object_keys(this.result.PERSON_TYPE).length, currentType, oldPersonTypeId, i, input, options = [], label, delimiter = false;
            ...
            if (personTypesCount > 2) {
                ...
                for (i in this.result.PERSON_TYPE) if (this.result.PERSON_TYPE.hasOwnProperty(i)) node.appendChild(BX.create("INPUT", {
                    props: {
                        type: "hidden",
                        name: "PERSON_TYPE",
        ...
        getProfilesControl: function(node) {
            var profilesLength = BX.util.object_keys(this.result.USER_PROFILES).length, i, label, options = [], profileChangeInput, input;
            if (profilesLength) {
                if (this.params.ALLOW_USER_PROFILES == "Y") {
                    ...
                    if (profilesLength > 1 || this.params.ALLOW_NEW_PROFILE == "Y") {
                        ...
                        if (this.params.ALLOW_NEW_PROFILE == "Y") options.unshift(BX.create("OPTION", {
                            props: {
                                value: 0
                            },
        ...
        editPaySystemBlock: function(active) {
            ...
            if (active) this.editActivePaySystemBlock(true); else this.editFadePaySystemBlock();
            this.initialized.paySystem = true;
        },
        editActivePaySystemBlock: function(activeNodeMode) {
            var node = activeNodeMode ? this.paySystemBlockNode : this.paySystemHiddenBlockNode, paySystemContent, paySystemNode;
        ...
        createPaySystemItem: function(item) {
            var checked = item.CHECKED == "Y", logotype, logoNode, paySystemId = parseInt(item.ID), title, label, itemNode;
            logoNode = BX.create("DIV", {
                props: {
            ...
            if (this.params.SHOW_PAY_SYSTEM_LIST_NAMES == "Y") title = BX.create("DIV", {
                props: {
            ...
            itemNode = BX.create("DIV", {
                props: {
                    className: "bx-soa-pp-company col-xs-6"
                },
        ...
        editPaySystemInfo: function(paySystemNode) {
            if (!this.result.PAY_SYSTEM || this.result.PAY_SYSTEM.length == 0 && this.result.PAY_FROM_ACCOUNT != "Y") return;
            ...
            if (currentPaySystem) {
                ...
                if (this.params.SHOW_PAY_SYSTEM_INFO_NAME == "Y") subTitle = BX.create("DIV", {
                    props: {
            ...
            if (innerPs && extPs) delimiter = BX.create("HR", {
                props: {
                    className: "bxe-light"
                }
            });
        ...
        getInnerPaySystem: function() {
            if (!this.result.CURRENT_BUDGET_FORMATED || !this.result.PAY_CURRENT_ACCOUNT || !this.result.INNER_PAY_SYSTEM) return;
            var accountOnly = this.params.ONLY_FULL_PAY_FROM_ACCOUNT && this.params.ONLY_FULL_PAY_FROM_ACCOUNT == "Y", isSelected = this.result.PAY_CURRENT_ACCOUNT && this.result.PAY_CURRENT_ACCOUNT == "Y", paySystem = this.result.INNER_PAY_SYSTEM, logotype, logoNode, subTitle, label, title, hiddenInput, htmlString, innerPsDesc;
            ...
            htmlString = BX.util.htmlspecialchars(this.params.MESS_INNER_PS_BALANCE) + ' <b class="wsnw">' + this.result.CURRENT_BUDGET_FORMATED + "</b><br />" + (accountOnly ? BX.message("SOA_PAY_ACCOUNT3") : "");
        ...
        editFadePaySystemContent: function(node) {
            var selectedPaySystem = this.getSelectedPaySystem(), errorNode = this.paySystemHiddenBlockNode.querySelector("div.alert.alert-danger"), warningNode = this.paySystemHiddenBlockNode.querySelector("div.alert.alert-warning.alert-show"), addedHtml = "", logotype, imgSrc;
            if (errorNode) node.appendChild(errorNode.cloneNode(true)); else this.getErrorContainer(node);
        ...
        getSelectedPaySystem: function() {
            var paySystemCheckbox = this.paySystemBlockNode.querySelector("input[type=checkbox][name=PAY_SYSTEM_ID]:checked"), currentPaySystem = null, paySystemId, i;
            if (!paySystemCheckbox) paySystemCheckbox = this.paySystemHiddenBlockNode.querySelector("input[type=checkbox][name=PAY_SYSTEM_ID]:checked");
            if (!paySystemCheckbox) paySystemCheckbox = this.paySystemHiddenBlockNode.querySelector("input[type=hidden][name=PAY_SYSTEM_ID]");
        ...
        isSelectedInnerPayment: function() {
            ...
            if (!innerPaySystemCheckbox) innerPaySystemCheckbox = this.paySystemHiddenBlockNode.querySelector("input[type=checkbox][name=PAY_CURRENT_ACCOUNT]");
        ...
        selectPaySystem: function(event) {
            ...
            var target = event.target || event.srcElement, innerPaySystemSection = this.paySystemBlockNode.querySelector("div.bx-soa-pp-inner-ps"), innerPaySystemCheckbox = this.paySystemBlockNode.querySelector("input[type=checkbox][name=PAY_CURRENT_ACCOUNT]"), fullPayFromInnerPaySystem = this.result.TOTAL && parseFloat(this.result.TOTAL.ORDER_TOTAL_LEFT_TO_PAY) === 0;
        ...
        editDeliveryBlock: function(active) {
            ...
            if (active) this.editActiveDeliveryBlock(true); else this.editFadeDeliveryBlock();
            this.checkPickUpShow();
            this.initialized.delivery = true;
        },
        editActiveDeliveryBlock: function(activeNodeMode) {
            var node = activeNodeMode ? this.deliveryBlockNode : this.deliveryHiddenBlockNode, deliveryContent, deliveryNode;
        ...
        editDeliveryInfo: function(deliveryNode) {
            ...
            }), currentDelivery, logotype, name, logoNode, subTitle, label, title, price, period, clear, infoList, extraServices, extraServicesNode;
            ...
            if (this.params.SHOW_DELIVERY_INFO_NAME == "Y") subTitle = BX.create("DIV", {
                props: {
                    className: "bx-soa-pp-company-subTitle"
                },
                text: name
        ...
        getDeliveryPriceNodes: function(delivery) {
            ...
            if (typeof delivery.DELIVERY_DISCOUNT_PRICE !== "undefined" && parseFloat(delivery.DELIVERY_DISCOUNT_PRICE) != parseFloat(delivery.PRICE)) {
                if (parseFloat(delivery.DELIVERY_DISCOUNT_PRICE) > parseFloat(delivery.PRICE)) priceNodesArray = [ delivery.DELIVERY_DISCOUNT_PRICE_FORMATED ]; else priceNodesArray = [ delivery.DELIVERY_DISCOUNT_PRICE_FORMATED, BX.create("BR"), BX.create("SPAN", {
                    props: {
                        className: "bx-price-old"
                    },
        ...
        getDeliveryExtraServices: function(delivery) {
            var extraServices = [], brake = false, i, currentService, serviceNode, serviceName, input;
            for (i in delivery.EXTRA_SERVICES) {
                ...
                if (currentService.editControl.indexOf("this.checked") == -1) {
                    ...
                    serviceNode = BX.create("DIV", {
                        ...
                        html: currentService.editControl + (currentService.description.length ? '<div class="bx-soa-service-small">' + BX.util.htmlspecialchars(currentService.description) + "</div>" : "")
                    });
                    ...
                    serviceNode = BX.create("DIV", {
                        ...
                        children: [ BX.create("LABEL", {
                            html: currentService.editControl + BX.util.htmlspecialchars(currentService.name) + (currentService.price ? " (" + currentService.priceFormatted + ")" : "") + (currentService.description.length ? '<div class="bx-soa-service-small">' + BX.util.htmlspecialchars(currentService.description) + "</div>" : "")
                        }) ]
                    });
                }
        ...
        createDeliveryItem: function(item) {
            var checked = item.CHECKED == "Y", deliveryId = parseInt(item.ID), labelNodes = [ BX.create("INPUT", {
                props: {
            ...
            }) ], deliveryCached = this.deliveryCachedInfo[deliveryId], logotype, label, title, itemNode, logoNode;
            logoNode = BX.create("DIV", {
                props: {
                    className: "bx-soa-pp-company-image"
                }
            ...
            if (item.PRICE >= 0 || typeof item.DELIVERY_DISCOUNT_PRICE !== "undefined") {
                labelNodes.push(BX.create("DIV", {
                    ...
                    text: typeof item.DELIVERY_DISCOUNT_PRICE !== "undefined" ? item.DELIVERY_DISCOUNT_PRICE_FORMATED : item.PRICE_FORMATED
                }));
                ...
                labelNodes.push(BX.create("DIV", {
                    ...
                    text: typeof deliveryCached.DELIVERY_DISCOUNT_PRICE !== "undefined" ? deliveryCached.DELIVERY_DISCOUNT_PRICE_FORMATED : deliveryCached.PRICE_FORMATED
                }));
            }
            label = BX.create("DIV", {
                props: {
                    className: "bx-soa-pp-company-graf-container" + (item.CALCULATE_ERRORS || deliveryCached && deliveryCached.CALCULATE_ERRORS ? " bx-bd-waring" : "")
                },
                children: labelNodes
            });
        ...
        editFadeDeliveryContent: function(node) {
            var selectedDelivery = this.getSelectedDelivery(), name = this.params.SHOW_DELIVERY_PARENT_NAMES != "N" ? selectedDelivery.NAME : selectedDelivery.OWN_NAME, errorNode = this.deliveryHiddenBlockNode.querySelector("div.alert.alert-danger"), warningNode = this.deliveryHiddenBlockNode.querySelector("div.alert.alert-warning.alert-show"), extraService, logotype, imgSrc, arNodes, i;
            if (errorNode && errorNode.innerHTML) node.appendChild(errorNode.cloneNode(true)); else this.getErrorContainer(node);
            if (warningNode && warningNode.innerHTML) node.appendChild(warningNode.cloneNode(true));
            ...
            } else node.appendChild(BX.create("STRONG", {
        ...
        selectDelivery: function(event) {
            ...
            var target = event.target || event.srcElement, actionSection = BX.hasClass(target, "bx-soa-pp-company") ? target : BX.findParent(target, {
                className: "bx-soa-pp-company"
            }), selectedSection = this.deliveryBlockNode.querySelector(".bx-soa-pp-company.bx-selected"), actionInput, selectedInput;
            if (BX.hasClass(actionSection, "bx-selected")) return BX.PreventDefault(event);
            if (actionSection) {
                actionInput = actionSection.querySelector("input[type=checkbox]");
                BX.addClass(actionSection, "bx-selected");
        ...
        getSelectedDelivery: function() {
            var deliveryCheckbox = this.deliveryBlockNode.querySelector("input[type=checkbox][name=DELIVERY_ID]:checked"), currentDelivery = false, deliveryId, i;
            if (!deliveryCheckbox) deliveryCheckbox = this.deliveryHiddenBlockNode.querySelector("input[type=checkbox][name=DELIVERY_ID]:checked");
            if (!deliveryCheckbox) deliveryCheckbox = this.deliveryHiddenBlockNode.querySelector("input[type=hidden][name=DELIVERY_ID]");
            if (deliveryCheckbox) {
        ...
        activatePickUp: function(deliveryName) {
            ...
            this.pickUpBlockNode.querySelector("h2.bx-soa-section-title").innerHTML = '<span class="bx-soa-section-title-count"></span>' + deliveryName;
        ...
        editPickUpBlock: function(active) {
            if (!this.pickUpBlockNode || !this.pickUpHiddenBlockNode || !BX.hasClass(this.pickUpBlockNode, "bx-active") || !this.result.DELIVERY) return;
            this.initialized.pickup = false;
            if (active) this.editActivePickUpBlock(true); else this.editFadePickUpBlock();
            this.initialized.pickup = true;
        },
        editActivePickUpBlock: function(activeNodeMode) {
            var node = activeNodeMode ? this.pickUpBlockNode : this.pickUpHiddenBlockNode, pickUpContent, pickUpContentCol, pickUpContentRow;
                    ...
                    if (this.pickUpMap && this.pickUpMap.geoObjects) {
                        ...
                        if (bounds && bounds.length) {
                            ...
                            if (!this.pickUpMapInitialized) {
                                this.pickUpMap.setBounds(bounds, {
                                    checkZoomRange: true
                                });
        ...
        editFadePickUpContent: function(pickUpContainer) {
            ...
            if (selectedPickUp) {
                ...
                if (selectedPickUp.ADDRESS) html += "<br /><strong>" + BX.message("SOA_PICKUP_ADDRESS") + ":</strong> " + BX.util.htmlspecialchars(selectedPickUp.ADDRESS);
                if (selectedPickUp.PHONE) html += "<br /><strong>" + BX.message("SOA_PICKUP_PHONE") + ":</strong> " + BX.util.htmlspecialchars(selectedPickUp.PHONE);
                if (selectedPickUp.SCHEDULE) html += "<br /><strong>" + BX.message("SOA_PICKUP_WORK") + ":</strong> " + BX.util.htmlspecialchars(selectedPickUp.SCHEDULE);
                if (selectedPickUp.DESCRIPTION) html += "<br /><strong>" + BX.message("SOA_PICKUP_DESC") + ":</strong> " + BX.util.htmlspecialchars(selectedPickUp.DESCRIPTION);
        ...
        getPickUpInfoArray: function(storeIds) {
            ...
            var arr = [], i;
            for (i = 0; i < storeIds.length; i++) if (this.result.STORE_LIST[storeIds[i]]) arr.push(this.result.STORE_LIST[storeIds[i]]);
            return arr;
        },
        getSelectedPickUp: function() {
            var pickUpInput = BX("BUYER_STORE"), currentPickUp, pickUpId, allStoresList = this.result.STORE_LIST, stores, i;
            if (pickUpInput) {
        ...
        checkPickUpShow: function() {
            ...
            if (currentDelivery && currentDelivery.STORE && currentDelivery.STORE.length) stores = this.getPickUpInfoArray(currentDelivery.STORE);
        ...
        geoLocationSuccessCallback: function(result, currentDelivery) {
            ...
            if (currentDelivery && currentDelivery.STORE) activeStores = this.getPickUpInfoArray(currentDelivery.STORE);
            if (activeStores.length >= this.options.pickUpMap.minToShowNearestBlock) this.editPickUpRecommendList(result.geoObjects.get(0));
            this.initPickUpPagination();
            this.editPickUpList(true);
            this.pickUpFinalAction();
        },
        initMaps: function() {
            this.mapsReady = true;
            var that = this, currentDelivery = this.getSelectedDelivery(), pickUpInput = BX("BUYER_STORE"), pickUpDefaults, geoLocation, provider, maxTime, i, storeInfoHtml, geoObj, propertyDefaults, activeStores;
            this.resizeMapContainers();
            if (currentDelivery && currentDelivery.STORE && currentDelivery.STORE.length) activeStores = this.getPickUpInfoArray(currentDelivery.STORE);
            if (activeStores && activeStores.length) {
                ...
                if (this.params.SHOW_NEAREST_PICKUP == "Y") {
                    ...
                    maxTime = this.options.pickUpMap.geoLocationMaxTime || 5e3;
                    geoLocation.get({
                        provider: provider,
                        timeOut: maxTime
                ...
                for (i = 0; i < activeStores.length; i++) {
                    ...
                    geoObj = new ymaps.Placemark([ activeStores[i].GPS_N, activeStores[i].GPS_S ], {
                        ...
                        balloonContentLayout: ymaps.templateLayoutFactory.createClass("<h3>{{ properties.storeTitle }}</h3>" + "{{ properties.storeBody|raw }}" + '<br /><a class="btn btn-sm btn-default" data-store="{{ properties.id }}">{{ properties.text }}</a>', {
                            build: function() {
                            ...
                            selectStoreByClick: function(e) {
                                ...
                                if (BX.Sale.OrderAjaxComponent.pickUpMap.container.isFullscreen()) BX.Sale.OrderAjaxComponent.pickUpMap.container.exitFullscreen();
                    ...
                    if (pickUpInput.value == activeStores[i].ID) geoObj.options.set("preset", "islands#redDotIcon");
                ...
                this.propsMap.events.add("click", BX.proxy(function(e) {
                    ...
                    if (this.propsMap.geoObjects.getLength() == 0) {
                        ...
                        placeMark.events.add([ "parentchange", "geometrychange" ], function() {
                            var orderDesc = BX("orderDescription"), coordinates = placeMark.geometry.getCoordinates(), ind, before, after, string;
                            if (orderDesc) {
                                ind = orderDesc.value.indexOf(BX.message("SOA_MAP_COORDS") + ":");
                                if (ind == -1) orderDesc.value = BX.message("SOA_MAP_COORDS") + ": " + coordinates[0] + ", " + coordinates[1] + "\r\n" + orderDesc.value; else {
        ...
        resizeMapContainers: function() {
            var pickUpMapContainer = BX("pickUpMap"), propertyMapContainer = BX("propsMap"), resizeBy = this.propsBlockNode, width, height;
            if (resizeBy && (pickUpMapContainer || propertyMapContainer)) {
                ...
                if (propertyMapContainer) propertyMapContainer.style.height = height + "px";
            }
        },
        ...
        editPickUpList: function(isNew) {
            ...
            }), buyerStoreInput = BX("BUYER_STORE"), selectedStore, container, i, found = false, recommendList, selectedDelivery, currentStore, storeNode;
            ...
            if (!recommendList || !recommendList.querySelector(".bx-soa-pickup-list-item.bx-selected")) {
                ...
                if (selectedDelivery && selectedDelivery.STORE) {
                    for (i = 0; i < selectedDelivery.STORE.length; i++) if (selectedDelivery.STORE[i] == selectedStore) found = true;
                }
            } else found = true;
        ...
        pickUpFinalAction: function() {
            var selectedDelivery = this.getSelectedDelivery(), deliveryChanged, buyerStoreInput = BX("BUYER_STORE");
            if (selectedDelivery) {
            ...
            if (deliveryChanged && this.pickUpBlockNode.id !== this.activeSectionId) this.editFadePickUpContent(BX.lastChild(this.pickUpBlockNode));
            if (deliveryChanged) BX.removeClass(this.pickUpBlockNode, "bx-step-completed");
            if (this.pickUpMap && this.pickUpMap.geoObjects) {
                this.pickUpMap.geoObjects.each(function(geoObject) {
                    if (geoObject.properties.get("id") == buyerStoreInput.value) geoObject.options.set({
                        preset: "islands#redDotIcon"
                    }); else if (parseInt(geoObject.properties.get("id")) > 0) geoObject.options.unset("preset");
                });
            }
        },
        getStoreInfoHtml: function(currentStore) {
            var html = "";
            if (currentStore.ADDRESS) html += BX.message("SOA_PICKUP_ADDRESS") + ": " + BX.util.htmlspecialchars(currentStore.ADDRESS) + "<br />";
            if (currentStore.PHONE) html += BX.message("SOA_PICKUP_PHONE") + ": " + BX.util.htmlspecialchars(currentStore.PHONE) + "<br />";
            if (currentStore.SCHEDULE) html += BX.message("SOA_PICKUP_WORK") + ": " + BX.util.htmlspecialchars(currentStore.SCHEDULE) + "<br />";
            if (currentStore.DESCRIPTION) html += BX.message("SOA_PICKUP_DESC") + ": " + BX.util.htmlspecialchars(currentStore.DESCRIPTION) + "<br />";
            return html;
        },
        createPickUpItem: function(currentStore, options) {
            options = options || {};
            var imgClassName = "bx-soa-pickup-l-item-detail", buttonClassName = "bx-soa-pickup-l-item-btn", logoNode, logotype, html, storeNode;
            if (this.params.SHOW_STORES_IMAGES == "Y") {
            ...
            storeNode = BX.create("DIV", {
                ...
                children: [ BX.create("DIV", {
                    ...
                    children: options.distance ? [ BX.util.htmlspecialchars(currentStore.ADDRESS), " ( ~" + options.distance + " " + BX.message("SOA_DISTANCE_KM") + " ) " ] : [ BX.util.htmlspecialchars(currentStore.ADDRESS) ]
                }), BX.create("DIV", {
                    props: {
                        className: imgClassName
                    },
                    children: [ logoNode, BX.create("DIV", {
                        props: {
        ...
        editPickUpRecommendList: function(geoLocation) {
            if (!this.pickUpPointsJSON || !this.pickUpPointsJSON.length || !geoLocation) return;
            var recommendList = BX.create("DIV", {
                props: {
                    className: "bx-soa-pickup-list recommend"
                }
            }), buyerStoreInput = BX("BUYER_STORE"), selectedDelivery = this.getSelectedDelivery(), length = this.pickUpPointsJSON.length < this.options.pickUpMap.nearestPickUpsToShow ? this.pickUpPointsJSON.length : this.options.pickUpMap.nearestPickUpsToShow, i, pointsGeoQuery, res, storeId, currentStore, distance, storeNode, container;
            for (i = 0; i < length; i++) {
                ...
                if (i == 0 && parseInt(selectedDelivery.ID) !== this.lastSelectedDelivery) buyerStoreInput.value = parseInt(currentStore.ID);
        ...
        selectStore: function(event) {
            ...
            if (BX.type.isString(event)) {
                ...
                storeItem = BX.hasClass(target, "bx-soa-pickup-list-item") ? target : BX.findParent(target, {
                    className: "bx-soa-pickup-list-item"
                });
            }
            if (storeItem && storeInput) {
                ...
                if (this.pickUpMap && this.pickUpMap.geoObjects) {
                    this.pickUpMap.geoObjects.each(BX.proxy(function(placeMark) {
                        if (placeMark.properties.get("id")) placeMark.options.unset("preset");
        ...
        getDeliverySortedArray: function(objDelivery) {
            ...
            arDelivery.sort(function(a, b) {
                ...
                if (sort === 0) return a.OWN_NAME.toLowerCase() > b.OWN_NAME.toLowerCase() ? 1 : a.OWN_NAME.toLowerCase() < b.OWN_NAME.toLowerCase() ? -1 : 0; else return sort;
            });
            return arDelivery;
        },
        editPropsBlock: function(active) {
            if (!this.propsBlockNode || !this.propsHiddenBlockNode || !this.result.ORDER_PROP) return;
            if (active) this.editActivePropsBlock(true); else this.editFadePropsBlock();
        ...
        editActivePropsBlock: function(activeNodeMode) {
            ...
            if (this.initialized.props) {
                ...
                if (this.propsBlockNode.getAttribute("data-visited") == "true") {
                    ...
                    if (validationErrors.length) BX.addClass(this.propsBlockNode, "bx-step-error"); else BX.removeClass(this.propsBlockNode, "bx-step-error");
                }
            }
        },
        ...
        editFadePropsContent: function(node) {
            if (!node || !this.locationsInitialized) return;
            var errorNode = this.propsHiddenBlockNode.querySelector(".alert"), personType = this.getSelectedPersonType(), fadeParamName, props, group, property, groupIterator, propsIterator, i, validPropsErrors;
            BX.cleanNode(node);
            if (errorNode) node.appendChild(errorNode.cloneNode(true));
            if (personType) {
            ...
            if (!props || props.length == 0) {
                ...
                while (group = groupIterator()) {
                    ...
                    while (property = propsIterator()) {
                        for (i = 0; i < props.length; i++) if (props[i] == property.getId() && property.getSettings()["IS_ZIP"] != "Y") this.getPropertyRowNode(property, node, true);
                    }
                }
            }
            if (this.propsBlockNode.getAttribute("data-visited") == "true") {
        ...
        editPropsItems: function(propsNode) {
            ...
            while (group = groupIterator()) {
                propsIterator = group.getIterator();
                while (property = propsIterator()) {
                    if (this.deliveryLocationInfo.loc == property.getId() || this.deliveryLocationInfo.zip == property.getId() || this.deliveryLocationInfo.city == property.getId()) continue;
        ...
        getPropertyRowNode: function(property, propsItemsContainer, disabled) {
            var propsItemNode = BX.create("DIV"), textHtml = "", propertyType = property.getType() || "", propertyDesc = property.getDescription() || "", label;
            if (disabled) {
                ...
                if (propertyDesc.length && propertyType != "STRING" && propertyType != "NUMBER" && propertyType != "DATE") textHtml += " <small>(" + BX.util.htmlspecialchars(propertyDesc) + ")</small>";
                label = BX.create("LABEL", {
                    attrs: {
            ...
            if (property.getId() in this.locations) {
                if (disabled) {
                    ...
                    for (i = 0; i < propRow.length; i++) {
                        ...
                        if (property.isMultiple()) insertedLoc.style.marginBottom = this.locationsTemplate == "search" ? "5px" : "20px";
                    ...
                    if (property.isMultiple()) {
                        propContainer.appendChild(BX.create("DIV", {
                            attrs: {
        ...
        addLocationProperty: function(e) {
            var target = e.target || e.srcElement, propId = target.getAttribute("data-prop-id"), lastProp = BX.previousSibling(target), insertedLoc, k, input, index = 0, prefix = "sls-", randomStr = BX.util.getRandomString(5);
            ...
            if (this.cleanLocations[propId]) {
                ...
                for (k in this.cleanLocations[propId].SCRIPT) if (this.cleanLocations[propId].SCRIPT.hasOwnProperty(k)) BX.evalGlobal(this.cleanLocations[propId].SCRIPT[k].JS.split("_key__").join("_" + index).replace(/sls-\d{5}/g, prefix + randomStr));
        ...
        insertDateProperty: function(property, propsItemNode, disabled) {
            ...
            if (disabled) {
                ...
                if (prop) {
                    ...
                    for (i = 0; i < dateInputs.length; i++) if (dateInputs[i].value && dateInputs[i].value.length) values.push(dateInputs[i].value);
        ...
        insertFileProperty: function(property, propsItemNode, disabled) {
            ...
            if (disabled) {
                ...
                if (propContainer) propsItemNode.appendChild(propContainer); else {
                    propContainer = BX.create("DIV", {
                        props: {
                            className: "soa-property-container"
                        }
        ...
        insertYNProperty: function(property, propsItemNode, disabled) {
            ...
            if (disabled) {
                ...
                if (prop) {
                    ...
                    for (i = 0; i < inputs.length; i += 2) values.push(inputs[i].checked ? BX.message("SOA_YES") : BX.message("SOA_NO"));
                    propsItemNode.innerHTML += this.valuesToString(values);
                }
            } else {
                propContainer = BX.create("DIV", {
        ...
        alterProperty: function(settings, propContainer) {
            var divs = BX.findChildren(propContainer, {
                tagName: "DIV"
            }), i, textNode, inputs, del, add, fileInputs, accepts, fileTitles;
            ...
            if (settings.TYPE == "FILE") {
                ...
                for (i = 0; i < fileTitles.length; i++) {
                    BX.bind(fileTitles[i], "click", function(e) {
                        var target = e.target || e.srcElement, fileInput = target && target.nextSibling && target.nextSibling.nextSibling;
            ...
            if (add.length) {
                ...
                BX.bind(add, "click", BX.proxy(function(e) {
                    var target = e.target || e.srcElement, targetContainer = BX.findParent(target, {
                        tagName: "div",
                        className: "soa-property-container"
                    }), del = targetContainer.querySelector("label"), add = targetContainer.querySelectorAll("input[type=button]"), textInputs = targetContainer.querySelectorAll("input[type=text]"), textAreas = targetContainer.querySelectorAll("textarea"), divs = BX.findChildren(targetContainer, {
                        tagName: "DIV"
                    });
                    ...
                    if (settings.TYPE == "FILE") {
                        ...
                        BX.bind(fileTitles[fileTitles.length - 1], "click", function(e) {
                            var target = e.target || e.srcElement, fileInput = target && target.nextSibling && target.nextSibling.nextSibling;
                            if (fileInput) setTimeout(function() {
        ...
        alterDateProperty: function(settings, inputText) {
            ...
            BX.bind(addon, "click", BX.delegate(function(e) {
                var target = e.target || e.srcElement, parentNode = BX.findParent(target, {
                    tagName: "DIV",
        ...
        isValidForm: function() {
            ...
            var regionErrors = this.isValidRegionBlock(), propsErrors = this.isValidPropertiesBlock(), navigated = false, tooltips, i;
        ...
        isValidRegionBlock: function() {
            ...
            var regionProps = this.orderBlockNode.querySelectorAll(".bx-soa-location-input-container[data-property-id-row]"), regionErrors = [], id, arProperty, data, i;
        ...
        isValidPropertiesBlock: function(excludeLocation) {
            if (!this.options.propertyValidation) return [];
            var props = this.orderBlockNode.querySelectorAll(".bx-soa-customer-field[data-property-id-row]"), propsErrors = [], id, propContainer, arProperty, data, i;
            for (i = 0; i < props.length; i++) {
        ...
        bindValidation: function(id, propContainer) {
            ...
            var arProperty = this.validation.properties[id], data = this.getValidationData(arProperty, propContainer), i, k;
            if (data && data.inputs && data.action) {
                for (i = 0; i < data.inputs.length; i++) {
                    if (BX.type.isElementNode(data.inputs[i])) BX.bind(data.inputs[i], data.action, BX.proxy(function() {
                        this.isValidProperty(data);
                    }, this)); else for (k = 0; k < data.inputs[i].length; k++) BX.bind(data.inputs[i][k], data.action, BX.proxy(function() {
                        this.isValidProperty(data);
                    }, this));
                }
            }
        ...
        showValidationResult: function(inputs, errors) {
            ...
            var input0 = BX.type.isElementNode(inputs[0]) ? inputs[0] : inputs[0][0], formGroup = BX.findParent(input0, {
                tagName: "DIV",
                className: "form-group"
            }), label = formGroup.querySelector("label"), tooltipId, inputDiv, i;
            ...
            for (i = 0; i < inputs.length; i++) {
                ...
                if (errors[i] && errors[i].length) BX.addClass(inputDiv, "has-error"); else BX.removeClass(inputDiv, "has-error");
            }
            if (errors.length) this.showErrorTooltip(tooltipId, label, errors.join("<br />")); else this.closeErrorTooltip(tooltipId);
        },
        validateString: function(input, arProperty, fieldName) {
            if (!input || !arProperty) return [];
            var value = input.value, errors = [], name = BX.util.htmlspecialchars(arProperty.NAME), field = !!fieldName ? BX.message("SOA_FIELD") + ' "' + name + '"' : BX.message("SOA_FIELD"), re;
            if (arProperty.MULTIPLE == "Y") return errors;
            if (arProperty.REQUIRED == "Y" && value.length == 0) errors.push(field + " " + BX.message("SOA_REQUIRED")); else {
                if (arProperty.MINLENGTH && arProperty.MINLENGTH > value.length) errors.push(BX.message("SOA_MIN_LENGTH") + ' "' + name + '" ' + BX.message("SOA_LESS") + " " + arProperty.MINLENGTH + " " + BX.message("SOA_SYMBOLS"));
                if (arProperty.MAXLENGTH && arProperty.MAXLENGTH < value.length) errors.push(BX.message("SOA_MAX_LENGTH") + ' "' + name + '" ' + BX.message("SOA_MORE") + " " + arProperty.MAXLENGTH + " " + BX.message("SOA_SYMBOLS"));
        ...
        validateLocation: function(input, arProperty, fieldName) {
            ...
            var parent = BX.findParent(input, {
                tagName: "DIV",
                className: "form-group"
            }), value = this.getLocationString(parent), errors = [], field = !!fieldName ? BX.message("SOA_FIELD") + ' "' + BX.util.htmlspecialchars(arProperty.NAME) + '"' : BX.message("SOA_FIELD");
            if (arProperty.MULTIPLE == "Y" && arProperty.IS_LOCATION !== "Y") return errors;
            if (arProperty.REQUIRED == "Y" && (value.length == 0 || value == BX.message("SOA_NOT_SPECIFIED"))) errors.push(field + " " + BX.message("SOA_REQUIRED"));
            return errors;
        },
        validateCheckbox: function(input, arProperty, fieldName) {
            if (!input || !arProperty) return [];
            var errors = [], field = !!fieldName ? BX.message("SOA_FIELD") + ' "' + BX.util.htmlspecialchars(arProperty.NAME) + '"' : BX.message("SOA_FIELD");
            if (arProperty.MULTIPLE == "Y") return errors;
        ...
        validateNumber: function(input, arProperty, fieldName) {
            if (!input || !arProperty) return [];
            var value = input.value, errors = [], name = BX.util.htmlspecialchars(arProperty.NAME), field = !!fieldName ? BX.message("SOA_FIELD") + ' "' + name + '"' : BX.message("SOA_FIELD"), num, del;
            if (arProperty.MULTIPLE == "Y") return errors;
            ...
            if (value.length) {
                ...
                if (arProperty.MIN && parseFloat(arProperty.MIN) > parseFloat(value)) errors.push(BX.message("SOA_MIN_VALUE") + ' "' + name + '" ' + parseFloat(arProperty.MIN));
                if (arProperty.MAX && parseFloat(arProperty.MAX) < parseFloat(value)) errors.push(BX.message("SOA_MAX_VALUE") + ' "' + name + '" ' + parseFloat(arProperty.MAX));
        ...
        validateEnum: function(inputs, arProperty, fieldName) {
            ...
            var values = [], errors = [], i, field = !!fieldName ? BX.message("SOA_FIELD") + ' "' + BX.util.htmlspecialchars(arProperty.NAME) + '"' : BX.message("SOA_FIELD");
            if (arProperty.MULTIPLE == "Y") return errors;
        ...
        validateFile: function(inputs, arProperty, fieldName) {
            if (!inputs || !arProperty) return [];
            var errors = [], i, files = inputs.files || [], field = !!fieldName ? BX.message("SOA_FIELD") + ' "' + BX.util.htmlspecialchars(arProperty.NAME) + '"' : BX.message("SOA_FIELD"), defaultValue = inputs.previousSibling.value, file, fileName, splittedName, fileExtension;
            if (arProperty.MULTIPLE == "Y") return errors;
            if (arProperty.REQUIRED == "Y" && files.length == 0 && defaultValue == "" && !arProperty.DEFAULT_VALUE.length) errors.push(field + " " + BX.message("SOA_REQUIRED")); else {
                for (i = 0; i < files.length; i++) {
                    ...
                    if (arProperty.ACCEPT.length > 0 && (fileExtension.length == 0 || arProperty.ACCEPT.indexOf(fileExtension) == "-1")) errors.push(BX.message("SOA_BAD_EXTENSION") + ' "' + fileName + '" (' + BX.util.htmlspecialchars(arProperty.ACCEPT) + ")");
                    if (file.size > parseInt(arProperty.MAXSIZE)) errors.push(BX.message("SOA_MAX_SIZE") + ' "' + fileName + '" (' + this.getSizeString(arProperty.MAXSIZE, 1) + ")");
                }
            }
            return errors;
        },
        ...
        editTotalBlock: function() {
            ...
            var total = this.result.TOTAL, priceHtml = total.ORDER_PRICE_FORMATED, discText, valFormatted, i, curDelivery, deliveryError, deliveryValue, showOrderButton = this.params.SHOW_TOTAL_ORDER_BUTTON == "Y";
            BX.cleanNode(this.totalInfoBlockNode);
            if (this.options.showPriceWithoutDiscount) priceHtml += '<br /><span class="bx-price-old">' + total.PRICE_WITHOUT_DISCOUNT + "</span>";
            this.totalInfoBlockNode.appendChild(this.createTotalUnit("Ð’Ð°Ñˆ Ð·Ð°ÐºÐ°Ð·", '<a href="/personal/cart/" class="bx-soa-editstep">Ð˜Ð·Ð¼ÐµÐ½Ð¸Ñ‚ÑŒ</a>'));
            this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message("SOA_SUM_SUMMARY"), priceHtml));
            if (this.options.showOrderWeight) this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message("SOA_SUM_WEIGHT_SUM"), total.ORDER_WEIGHT_FORMATED));
            if (this.options.showDiscountPrice) {
                discText = BX.message("SOA_SUM_DISCOUNT");
                if (total.DISCOUNT_PERCENT_FORMATED && parseFloat(total.DISCOUNT_PERCENT_FORMATED) > 0) discText += total.DISCOUNT_PERCENT_FORMATED;
            ...
            if (parseFloat(total.DELIVERY_PRICE) >= 0 && this.result.DELIVERY.length) this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message("SOA_SUM_DELIVERY"), deliveryValue, {
                error: deliveryError
            }));
            ...
            } else this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message("SOA_SUM_IT"), total.ORDER_TOTAL_PRICE_FORMATED, {
                total: true
            }));
            if (parseFloat(total.PAY_SYSTEM_PRICE) >= 0 && this.result.DELIVERY.length) this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message("SOA_PAYSYSTEM_PRICE"), "~" + total.PAY_SYSTEM_PRICE_FORMATTED));
            if (!this.result.SHOW_AUTH) {
        ...
        editMobileTotalBlock: function() {
            if (this.result.SHOW_AUTH) BX.removeClass(this.mobileTotalBlockNode, "visible-xs"); else BX.addClass(this.mobileTotalBlockNode, "visible-xs");
        ...
        createTotalUnit: function(name, value, params) {
            ...
            return BX.create("DIV", {
                props: {
                    className: "bx-soa-cart-total-line" + (!!params.total ? " bx-soa-cart-total-line-total" : "")
                },
                children: [ BX.create("SPAN", {
                    props: {
        ...
        basketBlockScrollCheckEvent: function(e) {
            var target = e.target || e.srcElement, scrollLeft = target.scrollLeft, scrollRight = target.scrollWidth - (scrollLeft + target.clientWidth), parent = target.parentNode;
            if (scrollLeft == 0) BX.removeClass(parent, "bx-soa-table-fade-left"); else BX.addClass(parent, "bx-soa-table-fade-left");
            if (scrollRight == 0) BX.removeClass(parent, "bx-soa-table-fade-right"); else BX.addClass(parent, "bx-soa-table-fade-right");
        },
        basketBlockScrollCheck: function() {
            var scrollableNodes = this.orderBlockNode.querySelectorAll("div.bx-soa-table-fade"), parentNode, parentWidth, tableNode, tableWidth, i, scrollNode, scrollLeft, scrollRight, scrollable = false;
            for (i = 0; i < scrollableNodes.length; i++) {
                ...
                if (scrollable) {
                    ...
                    if (scrollLeft == 0) BX.removeClass(parentNode, "bx-soa-table-fade-left"); else BX.addClass(parentNode, "bx-soa-table-fade-left");
                    if (scrollRight == 0) BX.removeClass(parentNode, "bx-soa-table-fade-right"); else BX.addClass(parentNode, "bx-soa-table-fade-right");
        ...
        totalBlockScrollCheck: function() {
            ...
            var scrollTop = BX.GetWindowScrollPos().scrollTop, ghostTop = BX.pos(this.totalGhostBlockNode).top, ghostBottom = BX.pos(this.orderBlockNode).bottom, width;
            if (ghostBottom - this.totalBlockNode.offsetHeight < scrollTop + 20) BX.addClass(this.totalInfoBlockNode, "bx-soa-cart-total-bottom"); else BX.removeClass(this.totalInfoBlockNode, "bx-soa-cart-total-bottom");
        ...
        totalBlockResizeCheck: function() {
            if (!this.totalInfoBlockNode || !this.totalGhostBlockNode) return;
            if (BX.hasClass(this.totalInfoBlockNode, "bx-soa-cart-total-fixed")) this.totalInfoBlockNode.style.width = this.totalGhostBlockNode.offsetWidth + "px";
        },
        totalBlockFixFont: function() {
            var totalNode = this.totalInfoBlockNode.querySelector(".bx-soa-cart-total-line.bx-soa-cart-total-line-total"), buttonNode, target, objList = [];
            if (totalNode) {
            ...
            if (objList.length) BX.FixFontSize.init({
                objList: objList,
